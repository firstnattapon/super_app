...
def main():
    ...
    with st.expander("📖 Concept & Formulas", expanded=False):
        ...
        **Concept:** เชื่อมกำไรจากทุก Flywheel เข้าเป็น **ลูกโซ่** (Chain) — 
        ...
        st.markdown("#### 📐 สมการ Continuous Rollover")
        st.markdown(r"$$b_{new} = b_{old} + c \cdot \ln(P/t_{old}) - c' \cdot \ln(P/t_{new})$$")
        st.caption("ปรับ Baseline ให้ต่อเนื่องเมื่อเปลี่ยน fix_c และ re-center ราคา t")
        
        st.markdown("#### 💡 Extrinsic Value (Ev) — ค่า K จ่ายทิ้ง")
        st.markdown(r"$$\text{Extrinsic Value (Ev)} = \text{Premium} - \text{Intrinsic Value}$$")
        st.caption("มูลค่าทางเวลาที่จ่ายค่า LEAPS — เป็นต้นทุนที่ต้องชนะให้ได้จากระบบ Chain")

        st.markdown("#### 💰 Shannon Baseline")
        st.markdown(r"$$\text{Shannon} = \text{fix\_c} \times \ln\left(\frac{P_t}{P_0}\right)$$")
        st.caption("กำไร/ขาดทุนจาก Rebalancing ตามทฤษฎี Shannon's Demon")
...
def _render_portfolio_summary(data: dict, tickers_list: list, df: pd.DataFrame):
    ...
    pool_cf = float(data.get("global_pool_cf", 0.0))
...
def _render_ticker_cards(tickers_list: list):
    ...
    for i in range(0, len(tickers_list), cols_per_row):
        ...
        for j, col in enumerate(cols):
            ...
            net_val = float(state.get("net_pnl", 0.0))
...
def _render_engine_metrics(data: dict, tickers_list: list, active_ticker: str, active_t_data: dict):
    pool_cf       = float(data.get("global_pool_cf", 0.0))
    ev_reserve    = float(data.get("global_ev_reserve", 0.0))
    ...
    with st.container(border=True):
        ...
        (g1, g2, g3, g4, div, t1, t2, t3, t4, t5, t6, t7) = st.columns(
            [1.8, 1.8, 1.6, 1.6, 0.05, 1.3, 1.3, 1.3, 1.3, 1.3, 1.3, 0.8],
            gap="small"
        )
...
def _render_ticker_watchlist(tickers_list: list, active_idx: int):
    ...
    st.radio("Ticker", labels, index=active_idx, key="ticker_watchlist_radio", label_visibility="collapsed")
...
def _render_chain_engine_center(data: dict, tickers_list: list, selected_ticker: str, t_data: dict, idx: int):
    ...
    with hc2:
        if st.session_state.get("_pending_round") and st.session_state.get("_pending_ticker_name") == selected_ticker:
    ...
    with yf_col_status:
        if not _YF_AVAILABLE:
            ...
            st.markdown(
                f"<span style='font-size:12px;color:{color}'>📡 {selected_ticker}: <b>${info['price']:.2f}</b> "
                f"<span style='color:#64748b'>({info['source']})</span></span>", unsafe_allow_html=True)
        else:
    ...
    if preview_clicked and p_new > 0:
        preview = run_chain_round(
            ticker_state=state, p_new=p_new, hedge_ratio=hedge_ratio,
            ignore_hedge=ignore_hedge, ignore_surplus=ignore_surplus,
        )
        if preview:
    ...
    if is_pending:
        with st.container(border=True):
            ...
            new_c_after = r2c1.number_input("fix_c (after)", value=float(rd["c_after"]), step=100.0, format="%.0f", key="edit_c_after", help=f"Before: ${float(rd['c_before']):,.0f}")
            new_p_new   = r2c2.number_input("Price (new t)", value=float(rd["p_new"]), step=0.1, format="%.2f", key="edit_p_new", help=f"Before: ${float(rd['p_old']):,.2f}")
            new_b_after = r2c3.number_input("Baseline (after)", value=float(rd["b_after"]), step=0.1, format="%.2f", key="edit_b_after", help=f"Before: ${float(rd['b_before']):,.2f}")

            d1, d2, d3 = st.columns(3)
            d1.markdown(_delta_badge(float(rd["c_before"]), float(new_c_after), ",.0f"), unsafe_allow_html=True)
            d2.markdown(_delta_badge(float(rd["p_old"]), float(new_p_new), ",.2f"), unsafe_allow_html=True)
            d3.markdown(_delta_badge(float(rd["b_before"]), float(new_b_after), ",.2f"), unsafe_allow_html=True)
        ...
        with btn_col:
            if st.button("✅ COMMIT — บันทึกถาวร", type="primary", use_container_width=True):
                rd.update({
                    ...
                    "surplus":        float(new_surplus),
                    "c_after":        float(new_c_after),
                    "p_new":          float(new_p_new),
                    "b_after":        float(new_b_after),
        ...
        with st.container(border=True):
            st.caption(
                f"📌 {selected_ticker}  |  "
                f"fix_c ${float(state.get('fix_c', 0)):,.0f}  |  "
                f"Price ${float(state.get('price', 0)):,.2f}  |  "
                f"Baseline ${float(state.get('baseline', 0)):,.2f}  |  "
    ...
    with st.expander('🎱 Pool CF & Allocation', expanded=False):
        ...
        with st.form("add_pool_cf_form", clear_on_submit=True):
            ...
            if btn_add and amount > 0:
                data["global_pool_cf"] = float(data.get("global_pool_cf", 0.0)) + amount
        ...
        with st.form("record_harvest_form", clear_on_submit=True):
            ...
            if btn_harvest and h_amount > 0:
                data["global_pool_cf"] = float(data.get("global_pool_cf", 0.0)) + h_amount
    ...
    with st.expander("🚀 Deploy to Baseline", expanded=True):
        ...
        if submitted and d_amt > 0:
            if d_amt > pool_cf:
                ...
                data["global_pool_cf"] = pool_cf - d_amt
...
def _render_treasury_log(data: dict, filter_ticker: str = ""):
    ...
    tbl = [{
        ...
        "Amount": f"${float(e.get('amount', 0.0)):,.2f}",
        "Pool CF": f"${float(e.get('pool_cf_balance', 0.0)):,.0f}",
        "EV Res": f"${float(e.get('ev_reserve_balance', 0.0)):,.0f}",
        "Note":   e.get("note", ""),
    } for e in filtered]
...
def _render_consolidated_history(t_data: dict):
    ...
    if rounds:
        tbl = [{
            "Date": rd.get("date", "")[:10], 
            "Action": f"Scale +${float(rd.get('scale_up', 0)):,.0f}" if float(rd.get("scale_up", 0)) > 0 else ("Inject/Deploy" if "Injection" in str(rd.get("action", "")) else rd.get("action", "Round")),
            "Price": f"${float(rd.get('p_old',0)):,.2f} > ${float(rd.get('p_new',0)):,.2f}", 
            "fix_c": f"${float(rd.get('c_before',0)):,.0f} > ${float(rd.get('c_after',0)):,.0f}",
            "b": f"${float(rd.get('b_before',0)):,.2f} > ${float(rd.get('b_after',0)):,.2f}", 
...
def _calculate_and_plot_payoff(def_p: float, def_c: float, req: dict, data: dict = None):
    try:
        ...
        premium_call, premium_put = req["premium_call"], req["premium_put"] # For future EV inclusion
        ...
        st.error(f"⚠️ เกิดข้อผิดพลาดในการจำลองกราฟ (โปรดตรวจสอบค่าที่ป้อน): {e}")
...
def _render_sankey_by_ticker(data: dict):
    ...
    for t in tickers_list:
        ...
        s = {
            "ticker":   t.get("ticker", "???"),
            "shannon":  sum(float(r.get("shannon_profit", 0.0)) for r in rounds),
            "hedge":    sum(float(r.get("hedge_cost",     0.0)) for r in rounds),
            "surplus":  sum(float(r.get("surplus",        0.0)) for r in rounds),
            "scale_up": sum(float(r.get("scale_up",       0.0)) for r in rounds),
            "harvest":  sum(float(r.get("harvest_profit", 0.0)) for r in rounds),
            "injection":sum(float(r.get("scale_up", 0.0)) for r in rounds if "Injection" in str(r.get("action", ""))),
            "ev":       float(state.get("cumulative_ev", 0.0)),
            "fix_c":    float(state.get("fix_c", 0.0)),
        }
    ...
    selected  = st.multiselect("🔍 เลือก Ticker (ว่าง = ทั้งหมด)", all_names, default=[], key="sankey_ticker_filter")
    active    = [s for s in ticker_stats if s["ticker"] in selected] if selected else ticker_stats

    mode = st.radio("โหมด", ["📊 ภาพรวมพอร์ต", "🎯 รายละเอียด by Ticker"], horizontal=True, key="sankey_mode")
    ...
    st.divider()
    show_t = tickers_list if not selected else [t for t in tickers_list if t.get("ticker") in [s["ticker"] for s in active]]
...
def _render_sankey_aggregated(ticker_stats: list, data: dict):
    try:
        ...
        tot_sh = tot_ha = tot_hg = tot_sc = tot_ev = 0.0
        ...
        if tot_sc > 0:                lk(SU, SC, min(tot_sc, max(0.0, net_s)), "Auto Scale Up")
        ...
        m4.metric("Net Surplus", f"${net_s:,.0f}", delta_color="normal" if net_s >= 0 else "inverse")
        m5.metric("Scale Up 🚀", f"${tot_sc:,.0f}")
        m6.metric("Ev Burn 🔥",  f"${tot_ev:,.0f}", delta_color="inverse")

    except Exception as e:
        st.error(f"⚠️ เกิดข้อผิดพลาดในการสร้าง Sankey Chart: {e}")
...
def _render_sankey_per_ticker(ticker_stats: list):
    try:
            ...
            with cols[i % 2]:
                with st.container(border=True):
                    ...
                    fig_t = go.Figure(data=[go.Sankey(
                        node=dict(pad=12,thickness=18,line=dict(color="rgba(255,255,255,0.1)",width=0.5), label=NL,color=NC),
        ...
        st.error(f"⚠️ เกิดข้อผิดพลาดในการสร้าง Sankey per ticker: {e}")
...
def _render_ticker_state_overview(tickers_list: list, data: dict):
    ...
    for t in tickers_list:
        ...
        rows.append({
            "Ticker":   t.get("ticker","???"),
            "Price":    f"${float(state.get('price',0.0)):,.2f}",
            "fix_c":    f"${float(state.get('fix_c',0.0)):,.0f}",
            "Baseline": f"${float(state.get('baseline',0.0)):,.2f}",
            "Ev Burn":  f"${float(state.get('cumulative_ev',0.0)):,.2f}",
            "Net P&L":  f"${float(state.get('net_pnl',0.0)):,.2f}",
            "Σ Shannon":f"${sum(float(r.get('shannon_profit',0.0)) for r in rounds):,.2f}",
            "Σ Hedge":  f"${sum(float(r.get('hedge_cost',0.0)) for r in rounds):,.2f}",
            "Rounds":   len(rounds),
        })
    if rows:
