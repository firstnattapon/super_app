...
def fetch_yahoo_price(ticker: str) -> tuple:
    ...
    if not _YF_AVAILABLE:
        return 0.0, "error: yfinance ไม่ได้ติดตั้ง"
        ...
        if price <= 0 and "." not in ticker and len(ticker) <= 5:
            ...
            if price <= 0:
                ...
                if not hist_bk.empty:
                    ...
                    source = f"history_5d (.BK)"

        if price <= 0:
            return 0.0, f"error: ไม่พบราคา {ticker}"
...
# ----------------------------------------------------------
# TAB 1: Active Dashboard (helpers, not in main nav)
# ----------------------------------------------------------
...
def _render_pnl_waterfall(df: pd.DataFrame):
    ...
    fig_wf = go.Figure(go.Waterfall(
        ...
        increasing_marker_color="#00c853", decreasing_marker_color="#ff1744",
...
def _render_engine_tab(data: dict):
    ...
    with z_left:   _render_ticker_watchlist(tickers_list, active_idx)
    with z_center: _render_center_panels(data, tickers_list, active_ticker, active_t_data)
    with z_right:  _render_chain_engine_center(data, tickers_list, active_ticker, active_t_data, active_idx)
...
def _render_chain_engine_center(data: dict, tickers_list: list,
    ...
    # ── ORDER STRIP (6 cols: P New | Hedge× | No Hedge | No Surplus | 🔄 | Preview) ──
    with st.container(border=True):
        st.caption("⚡ Order Strip — ป้อนค่าแล้วกด Preview")
        sc1, sc2, sc3, sc4, sc5, sc6 = st.columns([2.5, 1.8, 1.1, 1.3, 0.7, 1.8])

        with sc1:
            p_new = st.number_input(
                "P New", min_value=0.01, value=default_p,
                step=1.0, key=pnew_key,
            )
        with sc2:
            hedge_ratio = st.number_input(
                "Hedge ×", min_value=0.0, value=default_hr,
                step=0.5, key=f"strip_hr_{idx}",
            )
        with sc3:
        ...
        with sc5:
            ...
            fetch_clicked = st.button(
                ...
                help="ดึงราคาล่าสุดจาก Yahoo Finance แล้วเติม P New อัตโนมัติ"
                     if _YF_AVAILABLE else "pip install yfinance ก่อนใช้งาน",
    ...
    # ── Yahoo fetch status (1 caption line, no extra row) ─────────────
    if yf_info_key in st.session_state:
        info = st.session_state[yf_info_key]
        if info["price"] > 0:
            st.caption(f"📡 {selected_ticker} ${info['price']:.2f} · {info['source']}")
        else:
            st.caption(f"❌ Yahoo: {info['source']}")

    # ── Handle fetch ──────────────────────────────────────────────────
    if fetch_clicked:
        with st.spinner(f"ดึงราคา {selected_ticker}..."):
    ...
    with st.expander("🎱 Pool CF & Allocation", expanded=False):
        ...
        if eligible_tickers:
            with st.form("extract_baseline_form", clear_on_submit=True):
                hc1, hc2 = st.columns([2, 1])
                with hc1:
                    ext_ticker_name = st.selectbox("Select Ticker",
                        options=[t.get("ticker") for t in eligible_tickers], key="extract_ticker")
                    ...
                    ext_amount      = st.number_input("Extract Amount ($)",
                        min_value=0.0, max_value=max_baseline, value=max_baseline, step=100.0)
                with hc2:
                    st.write(""); st.write(""); st.write(""); st.write("")
                ...
                if btn_extract and ext_amount > 0 and selected_t_obj:
                    ...
                    selected_t_obj.setdefault("rounds", []).append(dummy_round)
                    save_trading_data(data)
    ...
    with st.expander("🚀 Deploy to Baseline", expanded=True):
        ...
        with st.form("deploy_to_baseline_form", clear_on_submit=True):
            ...
            with f1:
                d_amt  = st.number_input("Amount ($)", min_value=0.0,
                    max_value=float(pool_cf) if pool_cf > 0 else 0.0, value=0.0, step=100.0)
        ...
        if submitted and d_amt > 0:
            if d_amt > pool_cf:
                ...
                t_data_deploy.setdefault("rounds", []).append(deploy_round)
                log_treasury_event(data, "Deploy", -d_amt,
...
def _render_ev_leaps_section(data: dict):
    ...
    with st.expander("🛡️ Manage Pool EV LEAPS (Income & Expenses)"):
        ...
        with col_a:
            alloc_amt = st.number_input("Allocate Amount ($)", min_value=0.0,
                max_value=float(pool_cf), step=100.0, key="alloc")
        with col_b:
        ...
        with col_c:
            pay_leaps_amt         = st.number_input("LEAPS Net Flow ($)", value=0.0, step=100.0,
                help="Negative (-) = Expense/Cost.")
...
def _render_treasury_log(data: dict, filter_ticker: str = ""):
    ...
    sel = st.selectbox("กรอง Ticker", options,
...
def _render_consolidated_history(t_data: dict):
    ...
    if rounds:
        tbl = [{
            "Date":       rd.get("date", "")[:10],
            "Action":     f"Scale +${rd.get('scale_up', 0):,.0f}" if rd.get("scale_up", 0) > 0
                          else ("Inject/Deploy" if "Injection" in rd.get("action", "")
                                else rd.get("action", "Round")),
...
def _render_payoff_profile_tab(data: dict):
    ...
    t_data = next((t for t in tickers_list if t["ticker"] == selected_ticker),
    ...
    if pending and st.session_state.get("_pending_ticker_name") == t_data.get("ticker"):
        ...
        st.success(
            f"🔗 **Connected State:** กราฟตั้งต้นจากค่า **Preview Calculation** ของ "
            f"`{t_data.get('ticker')}` | New Price: ${def_p:,.2f} | New fix_c: ${def_c:,.0f}")
    else:
        st.info(
            f"🟢 **Current State:** กราฟตั้งต้นจากข้อมูลปัจจุบันของ "
            f"`{t_data.get('ticker')}` | Price: ${def_p:,.2f} | fix_c: ${def_c:,.0f}")
...
def _render_sankey_by_ticker(data: dict):
    ...
    if "ภาพรวม" in mode: _render_sankey_aggregated(active, data)
    else:                  _render_sankey_per_ticker(active)
...
def _render_manage_data(data: dict):
                ...
                if new_ticker and init_price > 0 and init_c > 0:
                    ...
                    if new_ticker not in existing:
                        data.setdefault("tickers", []).append({
                            ...
                            "current_state": {"price": init_price, "fix_c": init_c,
    ...
    with st.expander("⚠️ ลบข้อมูลทั้งหมด", expanded=False):
        if st.button("DELETE ALL DATA", type="primary"):
            data.update({"tickers": [], "global_pool_cf": 0.0,
                          "global_ev_reserve": 0.0, "treasury_history": []})
            save_trading_data(data)
